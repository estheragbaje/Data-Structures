...
class LRUCache:
    ...
    def get(self, key):
        ...
        if key not in self.cacheStorage:
            return None
        # otherwise return the value associated with the key and move the node to the head of the storage List
        else:
            node = self.cacheStorage[key]["node"]
            self.storageList.move_to_front(node)
            
            return self.cacheStorage[key]["value"]
    ...
    def set(self, key, value):
        ...
        if key in self.cacheStorage:
          self.cacheStorage[key]["value"] = value
          key_node = self.cacheStorage[key]["node"]
          self.storageList.move_to_front(key_node)
        else:
        # check if the length of the node is up to the limit
            if self.currentNode == self.limit:
                ...
                popped_key = self.storageList.remove_from_tail()
                self.cacheStorage.pop(popped_key)
        # add the new key value pair to the cache_storage and cache_list head
                node = self.storageList.add_to_head(key)
                self.cacheStorage[key] = {"value": value, "node": node}
